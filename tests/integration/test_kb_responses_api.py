#!/usr/bin/env python3
"""
Test KB search using Responses API
"""
import os
import sys
...
from src.services.kb_search_responses import search_kb_responses, get_responses_kb_searcher

def main():
    print("="*60)
    print("Testing KB Search with Responses API")
    print("="*60)
    ...
    start = datetime.now()
    result = search_kb_responses(query, max_results=3)
    ...
    if result['success']:
        ...
        if result.get('citations'):
            ...
            for i, cit in enumerate(result['citations'][:3], 1):
                print(f"{i}. {cit.get('filename', 'Unknown')}")
                if cit.get('content'):
                    print(f"   Content: {cit['content'][:100]}...")
    else:
    ...
    print("\n" + "="*60)
    print("✅ KB search with Responses API is working!")
    print("="*60)
